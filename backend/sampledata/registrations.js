...
function createDataForCities(client,done,cod_prov,cod_mun,pop){
    // 5% of population
    var nPoints = Math.ceil(pop*0.005);
    ...
    client.query(query,[cod_prov,cod_mun,nPoints],
        ...
        for (var i=0;i<result.rows.length;i++){
            data.push({
                date_registered : chance.date({year: 2015}),
                name : chance.name(),
                latitude : result.rows[i].lat,
                longitude : result.rows[i].lng,
                email : chance.email(),
                age : randomIntFromInterval(18,50),
                prov : cod_prov,
            });
        }
...
function generateCSV(){
    ...
    json2csv({ data: data, fields: fields,del: ';' }, function(err, csv) {
        ...
        var fs = require('fs');
        fs.writeFile("user_registrations.csv",csv, function(err) {
            if(err) {
                return console.log(err);
            }

            console.log("The file user_registrations.csv was saved!");
        });
    });
